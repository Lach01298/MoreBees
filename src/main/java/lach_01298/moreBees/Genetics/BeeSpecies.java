...
public enum BeeSpecies implements IBeeDefinition {
	...
	EMERALD(BeeBranches.CRYSTAL, "Emerald", false, new Color(0x4adc73), new Color(0xffdc16)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			beeSpecies.addProduct(new ItemStack(MoreBeesItems.CombRock), 0.30f)
			.addProduct(new ItemStack(MoreBeesItems.EmeraldFrag), 0.15f)
			.setTemperature(EnumTemperature.WARM).setHumidity(EnumHumidity.NORMAL);
	...
	DIAMOND(BeeBranches.CRYSTAL, "Diamond", false, new Color(0x5decf5), new Color(0xffdc16)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			beeSpecies.addProduct(new ItemStack(MoreBeesItems.CombRock), 0.30f)
			.addProduct(new ItemStack(MoreBeesItems.DiamondFrag), 0.15f).setHasEffect()
			.setTemperature(EnumTemperature.WARM).setHumidity(EnumHumidity.NORMAL);
	...
	RADIOACTIVE(BeeBranches.RADIOACTIVE, "Radioactive", true, new Color(0x3e720c), new Color(0x999999)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			...
			{
			beeSpecies.addProduct(new ItemStack(MoreBeesItems.CombRock), 0.30f)
			.addProduct(OreDicPreferences.get("oreUranium",1), 0.1f)
	...
	URANIUM(BeeBranches.RADIOACTIVE, "Uranium", false, new Color(0x6ac013), new Color(0x999999)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			...
			{
			beeSpecies.addProduct(new ItemStack(MoreBeesItems.CombRock), 0.30f)
			.addProduct(OreDicPreferences.get("crushedUranium",1), 0.15f).setHasEffect()
	...
	PRISMARINE(BeeBranches.AQUATIC, "Prismarine", true, new Color(0x63ab9d), new Color(0xffdc16)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			...
			.addProduct(new ItemStack(Items.PRISMARINE_SHARD,2),0.2f).setHasEffect()
			.setTemperature(EnumTemperature.NORMAL).setHumidity(EnumHumidity.DAMP);
		...
		protected void registerMutations() 
		{
			...
			BeeManager.beeMutationFactory.createMutation(ObsidianBee, ClayBee , getTemplate(), 15).restrictBiomeType(BiomeDictionary.Type.WET);
	...
	APOCALYPTIC(BeeBranches.APOCALYTIC, "Apocalyptic", true, new Color(0xcb7204), new Color(0x830d0d)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			...
			.addProduct(new ItemStack(Items.BLAZE_POWDER),0.2f)
			.addProduct(new ItemStack(Items.GUNPOWDER),0.2f)
			.addProduct(new ItemStack(Items.GHAST_TEAR),0.05f)
			.setTemperature(EnumTemperature.HELLISH).setHumidity(EnumHumidity.ARID);
		}
	...
	NETHERSTAR(BeeBranches.WITHER, "wither", false, new Color(0xc1c1c1), new Color(0x3c3c3c)) {
		...
		protected void setSpeciesProperties(IAlleleBeeSpeciesBuilder beeSpecies) {
			...
			.addSpecialty(new ItemStack(MoreBeesItems.NetherFrag),0.05f).setHasEffect()
			.setTemperature(EnumTemperature.HELLISH).setHumidity(EnumHumidity.ARID);
		}
	...
	// forestry bees
	private static final IAlleleBeeSpecies [] hiveBees = {(IAlleleBeeSpecies) AlleleManager.alleleRegistry.getAllele("forestry.speciesForest"), 
	...
	private IBeeGenome genome;
	 ...
	 BeeSpecies(BeeBranches branch, String binomial, boolean dominant, Color primary, Color secondary) 
	{
	...
	public static void initBees() {
		for (BeeSpecies bee : values()) {
			...
			{
			//addon bees
			case LEAD:if(LoadMods.enableLead){bee.init();}break;
			case SILVER:if(LoadMods.enableSilver){bee.init();}break;
			case RADIOACTIVE:if(LoadMods.enableRadioactive){bee.init();}break;
			case URANIUM:if(LoadMods.enableUranium){bee.init();}break;
			case ALUMINIUM:if(LoadMods.enableAluminium){bee.init();}break;
			case RUBY:if(LoadMods.enableRuby){bee.init();}break;
			case SAPPHIRE:if(LoadMods.enableSapphire){bee.init();}break;
		...
		for (BeeSpecies bee : values()) 
		{
			...
			{
			//addon bees
			case LEAD:if(LoadMods.enableLead){bee.registerMutations();}break;
			case SILVER:if(LoadMods.enableSilver){bee.registerMutations();}break;
			case RADIOACTIVE:if(LoadMods.enableRadioactive){bee.registerMutations();}break;
			case URANIUM:if(LoadMods.enableUranium){bee.registerMutations();}break;
			case ALUMINIUM:if(LoadMods.enableAluminium){bee.registerMutations();}break;
			case RUBY:if(LoadMods.enableRuby){bee.registerMutations();}break;
			case SAPPHIRE:if(LoadMods.enableSapphire){bee.registerMutations();}break;
			
			
			default:bee.registerMutations();	
			}
